...
aluno* insere_aluno(aluno *v, int *n, aluno novo){
    ...
    aluno *aux = new (nothrow) aluno[v, (*n+1) * (sizeof(aluno))];
    ...
    for(int i = 0; i < *n; i++){ //pecorre tudo
        aux[i]= v[i]; // copia a variavel v para aux
    }
        
    aux[*n] = novo; //vai até n pq o "v" antigo ia até n-1
    
    (*n)++; //altero o tamanho do vetor
...
aluno* remove_aluno(aluno *v, int *n, int matricula){
    ...
    for(int i = 0; i < *n; i++){
        if(v[i].matricula == matricula){
            v[i] = v[*n-1]; //troca pelo ultimo
            
            aluno *aux = new (nothrow) aluno[v, (*n-1) * (sizeof(aluno))]; //realoca
